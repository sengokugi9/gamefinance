...
let gameState = {
    ...
    lastNewJobOfferTurn: null, lastOutstandingWorkTurn: null, lastWorkBonusTurn: null, lastWorkOTTurn: null, Rewardyourself: null, RewardyourselfByplay: null, smartphontbroked: null, investjoke: null, travelwithfrind: null, investwithfrind: null, lotterry: null, lastMinorIllnessTurn1: null, taxreturn: null, Freecateringcompany: null,
    lastMinorIllnessTurn: null, lastWorkMistakeTurn: null, workActionCount: 0,
    hasSugarGlider: false, // สถานะการรับเลี้ยงชูการ์ไกลเดอร์
    sugarGliderExpense: 0, // ค่าใช้จ่ายประจำจากการเลี้ยงชูการ์ไกลเดอร์
    sugarGliderHappinessPerMonth: 0, // ความสุขเพิ่มต่อเดือน
    sugarGliderStressPerMonth: 0, // ความเครียดลดต่อเดือน
};
...
const events = [
    { type: 'good', prob: 0.40, events: [ //0.40
        { name: 'โครงการอบรมการเงินฟรีจากมหาวิทยาลัย' , decision: true, desc: 'มีโควต้าให้บุคคลทั่วไป 200 คน หัวหน้าเสนอมอบโควต้าให้แก่คุณ', condition: () => gameState.knowledge >= 55 && (gameState.Trainingatuniversity === null || gameState.turn - gameState.Trainingatuniversity >= 8), acceptEffect: () => ({cash: -1200, knowledge: 5, desc: 'คุณได้รับความรู้การเงินเพิ่มเติม (-1200 บาท)'}), rejectEffect: () => ({  stress: 5, desc: 'คุณปฏิเสธโควต้าของหัวหน้า และไม่เข้าร่วมโครงการอบรม' })},
        { name: 'เพื่อนชวนลงทุน' , decision: true, desc: 'เพื่อนของคุณโทรมาชวนการลงทุนระยะสัั่นที่ให้ 3%-5% ต่อปีแต่ต้องรับความเสี่ยงที่เงินอาจจะหาย?', condition: () => gameState.turn > 8 && gameState.cash >= 10000 && (gameState.investwithfrind === null || gameState.turn - gameState.investjoke >= 12), acceptEffect: () => ({cash: 650, happiness: 3, desc: 'คุณได้รับผลตอบแทนจากการเก็งกำไรระยะสั่น (650 บาท)'}), rejectEffect: () => ({  stress: 5, desc: 'คุณปฏิเสธข้อเสนอในการลงทุน' })},
        { name: 'ไปเที่ยวต่างจังหวัดกับเพื่อน' , decision: true, desc: 'เพื่อนของคุณชวนไปเที่ยวในช่วงลดราคา (ลด 13%)', condition: () => gameState.turn > 5 && (gameState.travelwithfrind === null || gameState.turn - gameState.travelwithfrind >= 8), acceptEffect: () => ({ cash: -3880, happiness: 20, stress: -10, fatigue: -5, desc: 'ไปเที่นวต่างจังหวัดกับกลุ่มเพื่อนในราคาถูก (-3880 บาท)' }), rejectEffect: () => ({ happiness: -8, desc: 'คุณปฏิเสธกลุ่มเพื่อน'})},
        { name: 'รางวัลกับตัวเองที่แสนสนุก' , decision: true, desc: 'เนื่องจากทำงานหนัก คุณจึงอยากให้รางวัลกับตัวเองด้วยการเล่นเกม EldenRing', condition: () => gameState.turn > 3 && (gameState.RewardyourselfByplay === null || gameState.turn - gameState.RewardyourselfByplay >= 5), acceptEffect: () => ({ cash: -1400, happiness: 8, stress: -3, desc: 'คุณซื้อเกม EldenRing ที่คุณอยากเล่นเป็นรางวัลคลายเครียดสำหรับคุณ (-1400 บาท)' }), rejectEffect: () => ({ fatigue: 10, desc: 'คุณได้แต่ร้องไห้แล้วดูคลิปเกม EldenRing แล้วหวังว่าจะได้เล่นเกมนั้นในอนาคต'})},
        { name: 'รางวัลกับตัวเองที่หอมหวาน' , decision: true, desc: 'เนื่องจากทำงานหนัก คุณจึงอยากให้รางวัลกับตัวเอง', condition: () => gameState.turn > 3 && (gameState.Rewardyourself === null || gameState.turn - gameState.Rewardyourself >= 4), acceptEffect: () => ({ cash: -750, happiness: 5, fatigue: -5, desc: 'คุณซื้ออาหารชุดใหญ่สำหรับการทำงานที่แสนเหนื่อย (-750 บาท)' }), rejectEffect: () => ({ happiness: -8, desc: 'คุณยอมรับที่จะทนต่อไปโดยหวังว่าจะสบายในภายหลัง'})},
        { name: 'โอกาสเอาเงินเข้ากระเป๋า' , decision: true, desc: 'หัวหน้าขอให้คุณทำงานล่วงเวลา 2 ชม. โดยจะให้รางวัลเล็กๆกับคุณหากคุณรับข้อเสนอ ', condition: () => gameState.turn > 6 && (gameState.lastWorkOTTurn === null || gameState.turn - gameState.lastWorkOTTurn >= 6), acceptEffect: () => ({ cash: 500, fatigue: 5, desc: 'คุณตอบรับคำขอจากหัวหน้า และทำงานล่วงเวลา ได้รับรางวัลจากหัวหน้า (+500 บาท)' }), rejectEffect: () => ({ stress: 5, desc: 'คุณปฏิเสธข้อเสนอและยืนยันที่จะไม่ทำต่อ'})},
        ...
        { name: 'ได้เงินคืนจากภาษี', effect: () => ({ cash: 1400, happiness: 3, stress: -3, desc: 'ยื่นภาษีถูกต้อง และมีสิทธิ์คืน (+1,400 บาท)' }), condition: () => gameState.fatigue > 50 && (gameState.taxreturn === null || gameState.turn - gameState.taxreturn >= 8)},
        { name: 'ผลงานยอดเยี่ยมจากความสุข' , decision: true, desc: 'ความสุขสูงทำให้คุณทำงานได้ยอดเยี่ยม! รับโบนัสเงินสด 2,000 บาท', condition: () => gameState.happiness >= 90 && (gameState.lastOutstandingWorkTurn === null || gameState.turn - gameState.lastOutstandingWorkTurn >= 5), acceptEffect: () => ({ cash: 2000, stress: -5, desc: 'รับโบนัสเงินสดจากผลงานยอดเยี่ยม (+2,000 บาท)' }), rejectEffect: () => ({ stress: -15, happiness: 3, fatigue: 8, desc: 'คุณยกความดีความชอบให้เพื่อนร่วมงาน'})}
    ]},
    { type: 'bad', prob: 0.57, events: [ //0.57
        { name: 'ซื้อหวย' , decision: true, desc: 'ชายขายลอตเตอรี่บอกว่ามีโอกาสถูก 22% คุณอาจเป็นหนึ่่งในนั้น', condition: () => gameState.turn > 5 && gameState.cash <= 50000 && (gameState.lotterry === null || gameState.turn - gameState.lotterry >= 7), acceptEffect: () => ({cash: -600, desc: 'คุณไม่ใช่หนึ่งในคนที่ถูกหวยรอบนี้ (-600 บาท)'}), rejectEffect: () => ({ desc: 'คุณปฏิเสธการซื้อลอตเตอรี่' })},
        { name: 'เพื่อนเก่าชวนลงทุน' , decision: true, desc: 'เพื่อนของคุณโทรมาชวนการลงทุนระยะยาวที่ให้ 17% ต่อปีโดยไม่มีความเสี่ยง', condition: () => gameState.turn > 8 && gameState.cash >= 10000 && (gameState.investjoke === null || gameState.turn - gameState.investjoke >= 8), acceptEffect: () => ({cash: -10000, knowledge: 3, stress: 10, desc: 'คุณโดนหลอกในการลงทุนซึ่งรู้ภายหลังว่าเป็นการแชร์ลูกโซ่ (-10000 บาท)'}), rejectEffect: () => ({  stress: 5, desc: 'คุณปฏิเสธข้อเสนอในการลงทุน' })},
        { name: 'การรับชมที่ต้องใช้จ่าย' , decision: true, desc: 'ช่วงนี้โทรศัพท์ของคุณดับบ่อยๆระหว่างดู TikTok คุณเลยคิดว่าอาจจะถึงเวลาซื้อใหม่?', condition: () => gameState.turn > 3 && (gameState.smartphontbroked === null || gameState.turn - gameState.smartphontbroked >= 8), acceptEffect: () => ({cash: -7990, happiness: 8, desc: 'คุณได้รับโทรศัพท์เครื่องใหม่ (-7990 บาท)'}), rejectEffect: () => ({ cash: -1400, stress: 8, fatigue: 15, desc: 'คุณได้ซ่อมโทรศัพท์ (-1400 บาท)' })},
        { name: 'ป่วยเล็กน้อย' , decision: true, desc: 'จ่ายค่ารักษาพยาบาล 1,500 บาท หรือยอมทนป่วย?', condition: () => gameState.turn > 3 && (gameState.lastMinorIllnessTurn === null || gameState.turn - gameState.lastMinorIllnessTurn >= 6), acceptEffect: () => ({cash: -(gameState.hasInsurance ? 150 : 1500), desc: `จ่ายค่ารักษาพยาบาล ( -${(gameState.hasInsurance ? 150 : 1500).toLocaleString()} บาท ${gameState.hasInsurance ? ' ประกัน Co-pay 10% ' : ''})`}), rejectEffect: () => ({ happiness: -3, stress: 8, fatigue: 25, desc: 'ยอมทนป่วย' }) },
        ...
        { name: 'ไข้หวัดใหญ่เล่นงาน' , decision: true, desc: 'ทำงานหนักจนร่างกายอ่อนแอ จ่ายค่ารักษาพยาบาล 5,500 บาท หรือยอมทนป่วย?', condition: () => gameState.turn > 1 && gameState.cash >= 30000 && (gameState.lastMinorIllnessTurn1 === null || gameState.turn - gameState.lastMinorIllnessTurn1 >= 12), acceptEffect: () => ({cash: -(gameState.hasInsurance ? 559 : 5500), desc: `จ่ายค่ารักษาพยาบาล ( -${(gameState.hasInsurance ? 559 : 5500).toLocaleString()} บาท ${gameState.hasInsurance ? ' ประกัน Co-pay 10% ' : ''})`}), rejectEffect: () => ({ happiness: -5, stress: 12, fatigue: 28, desc: 'ยอมทนป่วย' }) },

    ]},
    { type: 'rare', prob: 0.03, events: [ //0.03
        { name: 'บริษัทจัดเลี้ยงฟรี', effect: () => ({ happiness: 3, fatigue: -18, stress: -5, desc: 'ฉลองยอดขายทะลุเป้า (ความเคลียดลดลงอย่างมาก)' }), condition: () => gameState.turn > 20 && (gameState.Freecateringcompany === null || gameState.turn - gameState.Freecateringcompany >= 25)},
        ...
        { name: 'ข้อเสนองานใหม่', requiredKnowledge: 58,  effect: () => { gameState.monthlyIncome += 2000; return { happiness: 20, stress: 18, fatigue: 10, desc: `ข้อเสนองานใหม่ (เงินเดือนเริ่มต้น ${gameState.monthlyIncome.toLocaleString()} บาท)` }; } }
    ]}
];
...
const updateUI = handleError(() => {
    ...
    if (DOMCache['goal']) DOMCache['goal'].textContent = gameState.goal ? `${gameState.goal.desc} (เหลือ ${gameState.goal.turnsRemaining} เทิร์น)` : 'ไม่มีภารกิจในขณะนี้';
...
if (DOMCache['history-log']) {
    const filteredHistory = gameState.history
        .filter(item => item.turn >= gameState.turn - 1)
        .sort((a, b) => b.id - a.id)
        .slice(0, CONFIG.maxHistoryItems);

    if (filteredHistory.length === 0) {
        DOMCache['history-log'].innerHTML = `
            <tr class="text-gray-400">
                <td class="px-4 py-2" colspan="4">ไม่พบการกระทำหรือเหตุการณ์</td>
            </tr>`;
    } else {
        DOMCache['history-log'].innerHTML = filteredHistory
            .map(item => {
                ...
                return `<tr class="history-${item.type} ${item.turn === gameState.turn ? 'text-white shadow-md' : 'text-gray-400'}" style="${item.turn === gameState.turn ? 'text-shadow: 0 0 5px rgba(59, 130, 246, 0.5)' : ''}">
    ...
    addHistory('error', `ไม่พบ element ID "history-log" ใน UI`);
}
...
const applyFinancials = handleError(async () => {
    ...
    if (gameState.hasSugarGlider) {
        totalExpenses += gameState.sugarGliderExpense;
        gameState.happiness = Math.max(0, Math.min(100, gameState.happiness + gameState.sugarGliderHappinessPerMonth));
        gameState.stress = Math.max(0, Math.min(100, gameState.stress + gameState.sugarGliderStressPerMonth));
        addHistory('financial', `เดือนที่ ${gameState.month}: ค่าใช้จ่ายเลี้ยงชูการ์ไกลเดอร์ (-${gameState.sugarGliderExpense.toLocaleString()} บาท)`, -gameState.sugarGliderExpense);
        addHistory('event', `ชูการ์ไกลเดอร์ทำให้คุณมีความสุข (+${gameState.sugarGliderHappinessPerMonth}) และลดความเครียด (${gameState.sugarGliderStressPerMonth})`);
    }
    ...
    for (const eventType of events) {
        ...
        if (rand <= cumulative) {
            ...
            if (event.decision) {
                ...
                if (event.name === 'ได้เงินคืนจากภาษี') {
                    gameState.taxreturn = gameState.turn; // Update cooldown tracker for work bonus
                }
                if (event.name === 'บริษัทจัดเลี้ยงฟรี') {
                    gameState.Freecateringcompany = gameState.turn; // Update cooldown tracker for work bonus
                }
            ...
            if (effect) {
                ...
                if (event.name === 'เพื่อนเก่าชวนลงทุน') {
                    gameState.investjoke = gameState.turn; // Update cooldown tracker for work mistake
                }
                if (event.name === 'ไปเที่ยวต่างจังหวัดกับเพื่อน') {
                    gameState.travelwithfrind = gameState.turn; // Update cooldown tracker for work mistake
                }
                if (event.name === 'เพื่อนชวนลงทุน') {
                    gameState.investwithfrind = gameState.turn; // Update cooldown tracker for work mistake
                }
                if (event.name === 'ซื้อหวย') {
                    gameState.lotterry = gameState.turn; // Update cooldown tracker for work mistake
                }
                if (event.name === 'ไข้หวัดใหญ่เล่นงาน') {
                    gameState.lastMinorIllnessTurn1 = gameState.turn; // Update cooldown tracker for work mistake
                }
                if (event.name === 'โครงการอบรมการเงินฟรีจากมหาวิทยาลัย') {
                    gameState.Trainingatuniversity = gameState.turn; // Update cooldown tracker for work mistake
                }
...
const showSubActions = handleError(async (title, actionList) => {  // เปลี่ยน parameter เป็น actionList เพื่อชัดเจนว่าเป็น array
    ...
    const actionDetails = {
        'work-extra': { name: 'งานพิเศษ', desc: 'ทำงานพิเศษเพื่อรับเงินเพิ่ม', effects: [{ label: 'เงินสด', value: '+2,500 บาท', color: 'text-green-400' }, { label: 'ความเหนื่อย', value: '+20', color: 'text-red-400' }, { label: 'ความเครียด', value: '+10', color: 'text-yellow-400' }, { label: 'ความสุข', value: '-10', color: 'text-red-400' }, { label: 'คูลดาวน์', value: '2 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'work-freelance': { 
            name: 'งานฟรีแลนซ์', 
            desc: 'รับงานฟรีแลนซ์เพื่อรับเงินเพิ่มและทำสิ่งที่ชอบ', 
            effects: [
                { label: 'เงินสด', value: '2,000 – 5,000 บาท ', color: 'text-green-400' }, 
                { label: 'ความเหนื่อย', value: '+20', color: 'text-red-400' }, 
                { label: 'ความเครียด', value: '+10', color: 'text-yellow-400' }, 
                { label: 'ความสุข', value: '+5', color: 'text-green-400' }, 
                { label: 'คูลดาวน์', value: '3 เทิร์น', color: 'text-gray-400' }
            ], 
            bgColor: 'bg-[#1f2937]' 
        },
        'work-overtime': { name: 'ทำงานล่วงเวลา', desc: 'ทำงานล่วงเวลาเพื่อรับเงินมากขึ้น', effects: [{ label: 'เงินสด', value: '+4,500 บาท', color: 'text-green-400' }, { label: 'ความเหนื่อย', value: '+35', color: 'text-red-400' }, { label: 'ความเครียด', value: '+30', color: 'text-yellow-400' }, { label: 'ความสุข', value: '-20', color: 'text-red-400' }, { label: 'คูลดาวน์', value: '4 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'rest-relax': { name: 'ลาเพื่อพักผ่อน', desc: 'ใช้เวลาหนึ่งวันพักผ่อน ดูหนัง ฟังเพลง ไม่ออกไปไหน เติมพลังเล็ก ๆ', effects: [{ label: 'เงินสด', value: '-2,250 บาท', color: 'text-red-400' }, { label: 'ความเหนื่อย', value: '-15', color: 'text-green-400' }, { label: 'ความสุข', value: '+10', color: 'text-green-400' }, { label: 'ความเครียด', value: '-8', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '4 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'rest-vacation': { name: 'ไปเที่ยว', desc: 'ไปเที่ยวทริประยะไกล ฟื่นฟูร่างกาย', effects: [{ label: 'เงินสด', value: '-4,300 บาท', color: 'text-red-400' }, { label: 'ความเหนื่อย', value: '-20', color: 'text-green-400' }, { label: 'ความสุข', value: '+18 (+10 ในเทิร์นถัดไป)', color: 'text-green-400' }, { label: 'ความเครียด', value: '-15', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '5 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'rest-Party': { name: 'ปาร์ตี้กับเพื่อน', desc: 'ดื่ม กิน เที่ยวกับแก๊งเพื่อน ช่วยรีเฟรชอารมณ์', effects: [{ label: 'เงินสด', value: '-3,500 บาท', color: 'text-red-400' }, { label: 'ความเหนื่อย', value: '-5', color: 'text-green-400' }, { label: 'ความสุข', value: '+8', color: 'text-green-400' }, { label: 'ความเครียด', value: '-15', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '3 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'study-free': { name: 'เรียนรู้ด้วยตัวเอง', desc: 'การเรียนรู้ฟรี! ศึกษาเรียนรู้จากช่องทางอินเทอร์เน็ต', effects: [{ label: 'เงินสด', value: 'ฟรี!', color: 'text-white' }, { label: 'ความรู้', value: '2 - 5 ', color: 'text-green-400' }, { label: 'ความเหนื่อย', value: '+10', color: 'text-red-400' }, { label: 'ความสุข', value: '+3', color: 'text-green-400' }, { label: 'ความเครียด', value: '-5', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '2 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'study-saving': { 
            name: 'การออมเงิน', 
        ...
        'buy-essential': { name: 'ซื้อของจำเป็นขนาดกลาง', desc: 'ของจำเป็นขนาดกลางสำหรับชีวิตประจำวัน', effects: [{ label: 'เงินสด', value: '-2,000 บาท', color: 'text-red-400' }, { label: 'ความสุข', value: '+8', color: 'text-green-400' }, { label: 'ความเครียด', value: '-3', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '2 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'buy-luxury': { name: 'ซื้อของฟุ่มเฟือย', desc: 'ของฟุ่มเฟือยขนาดใหญ่เพื่อเพิ่มความสุข', effects: [{ label: 'เงินสด', value: '-8,000 บาท', color: 'text-red-400' }, { label: 'ความสุข', value: '+20', color: 'text-green-400' }, { label: 'ความเครียด', value: '-10', color: 'text-green-400' }, { label: 'ความเหนื่อย', value: '-5', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '4 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'buy-trendy': { 
            name: 'ซื้อสินค้าตามเทรน', 
            desc: 'สินค้าใหม่ๆ แสดงถึงความไม่ตกเทรน', 
            effects: [
                { label: 'เงินสด', value: '800 - 2,500 บาท', color: 'text-red-400' }, 
                { label: 'ความสุข', value: '+5', color: 'text-green-400' }, 
                { label: 'ความเครียด', value: '-8', color: 'text-green-400' }, 
                { label: 'คูลดาวน์', value: '2 เทิร์น', color: 'text-gray-400' }
            ], 
            bgColor: 'bg-[#1f2937]' ,
        },
        'buy-sugar-glider': {
            name: 'ซื้อชูการ์ไกลเดอร์ตัว Secret',
            desc: 'รับเลี้ยงชูการ์ไกลเดอร์ตัว Secret สายกินจุ',
            effects: [
                { label: 'เงินสด', value: '-5,000 บาท', color: 'text-red-400' },
                { label: 'ความสุข', value: '+15', color: 'text-green-400' },
                { label: 'ความเครียด', value: '-10', color: 'text-green-400' },
                { label: 'ค่าใช้จ่ายประจำ', value: '+1,200 บาท/เดือน', color: 'text-red-400' },
                ...(gameState.hasSugarGlider ? [{ label: 'สถานะ', value: 'คุณเลี้ยงชูการ์ไกลเดอร์อยู่แล้ว!', color: 'text-teal-400' }] : [])
            ],
            bgColor: 'bg-[#1f2937]'
        },
        'pay-debt': { name: 'ชำระหนี้', desc: 'ชำระหนี้ 2,000 บาท', effects: [{ label: 'เงินสด', value: '-2,000 บาท', color: 'text-red-400' }, { label: 'หนี้', value: '-2,000 บาท', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '3 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'deposit': { name: 'ฝากเงิน', desc: 'ฝากเงิน 1,000 บาท', effects: [{ label: 'เงินสด', value: '-1,000 บาท', color: 'text-red-400' }, { label: 'เงินฝาก', value: '+1,000 บาท', color: 'text-green-400' }, { label: 'คูลดาวน์', value: '2 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'withdraw': { name: 'ถอนเงิน', desc: 'ถอนเงิน 1,000 บาท', effects: [{ label: 'เงินสด', value: '+1,000 บาท', color: 'text-green-400' }, { label: 'เงินฝาก', value: '-1,000 บาท', color: 'text-red-400' }, { label: 'คูลดาวน์', value: '2 เทิร์น', color: 'text-gray-400' }], bgColor: 'bg-[#1f2937]' },
        'buy-insurance': {
    ...
    const cards = filteredActions.map(action => {
        ...
        const isDisabled = 
            (action.id === 'study-saving' && (gameState.savingKnowledge.cooldown > 0 || gameState.savingKnowledge.completed || !gameState.financeBasics.completed)) ||
            ...
            (action.id === 'buy-sugar-glider' && (gameState.hasSugarGlider || gameState.cash < 5000)); // เพิ่มเงื่อนไขปิดการ์ดเมื่อมีชูการ์ไกลเดอร์
        return `<div class="subaction-card mb-1 mt-2 px-6 pt-6 pb-10 rounded-md glassmorphism border border-gray-800 ${details.bgColor} shadow-md backdrop-blur-sm transition-all duration-200 ease-in-out ${isDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-2xl hover:bg-opacity-60 hover:border-[#34d399] hover:bg-[#1a2330] active:bg-opacity-20 active:scale-99 active:shadow-inner'}" data-action="${action.id}" style="${isDisabled ? 'pointer-events: none' : ''}">
    ...
    await Swal.fire({ 
        title, 
        html: `<div class="subaction-container ${isTwoColumns ? 'subaction-container-two-cols grid grid-cols-1 md:grid-cols-2 gap-4' : 'grid grid-cols-1 gap-4'} max-h-[70vh] overflow-y-auto px-2">${cards}</div>`, 
        showConfirmButton: false, 
        showCancelButton: true, 
        ...swalConfig, 
        customClass: { 
            ...swalConfig.customClass, 
            popup: `bg-gray-900 bg-opacity-80 text-white rounded-lg ${isTwoColumns ? 'max-w-3xl' : 'max-w-md'} w-full border border-gray-700 shadow-md backdrop-blur-md` 
        }, 
        didOpen: () => document.querySelectorAll('.subaction-card:not(.cursor-not-allowed)').forEach(card => card.addEventListener('click', async () => { 
            await Swal.close(); 
            await new Promise(r => setTimeout(r, 300)); 
            await handleSubAction(card.getAttribute('data-action')); 
        })) 
    });
}, 'showSubActions');
...
const handleSubAction = handleError(async actionId => {
    ...
    const actions = {
        'work-extra': { effect: { cash: 2500, fatigue: 20, stress: 10, happiness: -10 }, desc: `งานพิเศษ (+${(2500).toLocaleString()} บาท)` },
        'work-freelance': (() => {
            const cashAmount = Math.floor(Math.random() * (5000 - 2000 + 1)) + 2000;
            return { 
                effect: { cash: cashAmount, fatigue: 20, stress: 10, happiness: 5 }, 
                desc: `งานฟรีแลนซ์ (+${cashAmount.toLocaleString()} บาท)` 
            };
        })(),
        'work-overtime': { effect: { cash: 4500, fatigue: 35, stress: 30, happiness: -20 }, desc: `ทำงานล่วงเวลา (+${(4500).toLocaleString()} บาท)` },
        'rest-relax': { effect: { fatigue: -15, happiness: 10, stress: -8, cash: -2250 }, desc: `พักผ่อนที่บ้าน (-${(2250).toLocaleString()} บาท)` },
        'rest-vacation': { effect: { fatigue: -20, happiness: 18, stress: -15, cash: -4300 }, desc: `ไปเที่ยว (-${(4300).toLocaleString()} บาท)` },
        'rest-Party': { effect: { fatigue: -5, happiness: 8, stress: -15, cash: -3500 }, desc: `ปาร์ตี้กับเพื่อน (-${(3500).toLocaleString()} บาท)` },
        'study-free': (() => {
            const knowledgeGain = Math.floor(Math.random() * (5 - 2 + 1)) + 2;
            return { 
                effect: { knowledge: knowledgeGain, fatigue: 10, happiness: 3, stress: -5 }, 
                desc: `เรียนรู้ด้วยตัวเอง (ความรู้ +${knowledgeGain})` 
            };
        })(),
        ...
        'buy-essential': { effect: { cash: -2000, happiness: 8, stress: -3}, desc: `ซื้อของจำเป็นขนาดกลาง (-${(2000).toLocaleString()} บาท)` },
        'buy-luxury': { effect: { cash: -8000, happiness: 20, stress: -10, fatigue: -5 }, desc: `ซื้อของฟุ่มเฟือย (-${(8000).toLocaleString()} บาท)` },
        'buy-trendy': (() => {
            const cashSpent = -Math.floor(Math.random() * (2500 - 800 + 1)) - 800;
            return { 
                effect: { cash: cashSpent, happiness: 5, stress: -8 }, 
                desc: `ซื้อสินค้าตามเทรน (-${(-cashSpent).toLocaleString()} บาท)` 
            };
        })(),
        'buy-sugar-glider': {
            effect: !gameState.hasSugarGlider && gameState.cash >= 5000 ? {
                cash: -5000,
                happiness: 15,
                stress: -10
            } : null,
            desc: `รับเลี้ยงชูการ์ไกลเดอร์ตัว Secret สายกินจุ (-${(5000).toLocaleString()} บาท, ค่าใช้จ่ายประจำ +1,200 บาท/เดือน)`
        },
    ...
    } else if (actionId === 'buy-sugar-glider') {
        if (gameState.hasSugarGlider) return await showAlert('ข้อผิดพลาด', 'คุณเลี้ยงชูการ์ไกลเดอร์อยู่แล้ว!', 'error');
        if (gameState.cash < 5000) return await showAlert('ข้อผิดพลาด', `เงินสดไม่พอสำหรับรับเลี้ยงชูการ์ไกลเดอร์ (ต้องการ 5,000 บาท, มี ${gameState.cash.toLocaleString()} บาท)`, 'error');
        gameState.hasSugarGlider = true;
        gameState.sugarGliderExpense = 1200;
        gameState.sugarGliderHappinessPerMonth = 2;
        gameState.sugarGliderStressPerMonth = -1;
        addHistory('financial', `เริ่มรับเลี้ยงชูการ์ไกลเดอร์: ค่าใช้จ่ายประจำ +1,200 บาท/เดือน`);
    }
    ...
    if (effect.cash < 0 && gameState.cash + effect.cash < 0) {
        ...
        let alertText = `เงินสดไม่เพียงพอขาด ${shortfall.toLocaleString()} บาท`; 
        ...
        if (result.isConfirmed && showConfirm) {
            if (await tryWithdrawFromSavings(shortfall, desc)) {
                ...
                if (actionId === 'rest-vacation') {
                    gameState.vacationCooldown = 5;
                    gameState.happinessNextTurn = 10;
                    await showAlert('ไปเที่ยว!', 'คุณเลือกไปเที่ยว จะไม่สามารถกระทำใด ๆ ได้ใน 5 เทิร์นถัดไป', 'info');
                }
                if (actionId === 'work-freelance') {
                    gameState.workActionCount += 1;
                }
            ...
            if (loanResult.isConfirmed) {
                ...
                if (actionId === 'study-saving') {
                    ...
                    gameState.loanKnowledge.progress += 1;
                    gameState.loanKnowledge.cooldown = 4;
                } else if (actionId === 'work-freelance') {
                    gameState.workActionCount += 1;
                }
            ...
            addHistory('financial', `ยกเลิก ${desc} เนื่องจาก${gameState.loanKnowledge.completed ? 'หนี้ถึงขีดจำกัด' : 'ต้องเรียนรู้การกู้เงิน!'}`);
...
const applyActionEffect = handleError(async (effect, desc, actionId, borrowed = false) => {
    ...
    if (!['study-saving', 'study-finance-basics', 'study-insurance', 'study-loan', 'buy-insurance'].includes(actionId)) 
        gameState.actionCooldowns.set(actionId, actionId === 'work-overtime' ? 4 : actionId === 'rest-relax' ? 4 : actionId === 'rest-vacation' ? 5 : actionId === 'buy-luxury' ? 4 : actionId === 'work-freelance' ? 3 : actionId === 'rest-Party' ? 3 : 2);
    if (gameState.knowledge !== prevKnowledge) await checkUnlockedEvents();
    updateUI();
    checkGameStatus();
}, 'applyActionEffect');
...
document.addEventListener('DOMContentLoaded', handleError(() => {
    ...
    const buttons = [
        ...
        { id: 'rest-btn', actions: [{ id: 'rest-relax' }, { id: 'rest-vacation' }, { id: 'rest-Party' }], title: 'พักผ่อน' },
        ...
        { id: 'buy-btn', actions: [{ id: 'buy-essential' }, { id: 'buy-luxury' }, { id: 'buy-trendy' }, { id: 'buy-sugar-glider' }], title: 'ซื้อของ' },
        ...
        { id: 'work-btn', actions: [{ id: 'work-extra' }, { id: 'work-overtime' }, { id: 'work-freelance' }], title: 'ทำงาน' }
    ];
